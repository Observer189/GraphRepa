...
public: 
	...
	static std::shared_ptr<ProgramShader> make_from(std::string& vs_text, std::string& fs_text) {
		auto v = std::make_shared<VertexShader>(vs_text);
	...
	GLint GetUniformIdFallable(const char* name) {
		auto uloc = glGetUniformLocation(id, name);
		return uloc;
	}
	//Chech for -1 for error
...
{
public:
//	Scene(std::vector<GameObject> objects, Camera camera, GlobalLightSource gls, PointLightSource psl, ProjectorLightSource pjls) {
